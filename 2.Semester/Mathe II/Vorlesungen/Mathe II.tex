...
\begin{center}
    \ce{A -> B + C}
\end{center}
Die Wahrscheinlichkeit, dass ein Molekül \ce{A} zefällt, sei unabhängig davon, wie lange es schon existiert und wieviele andere Moleküle \ce{A} in der Nähe sind. (Keine gegenseitige Beeinflussung). Dann gilt:
\begin{eqnarray*}
    D = k\cdot N\cdot\Delta t
\end{eqnarray*}
$D$ ist die Anzahl der Zerfälle $N$ die Anzahl von \ce{A} Molekülen "Je mehr \ce{A}s da sind, desto mehr kann zerfallen." $\Delta t$ ist der Beobachtungszeitraum "Je länger zugesehen wird, desto mehr Zerfälle können beobachtet werden." $k$ ist eine Proportionalitätskonstante.\\
